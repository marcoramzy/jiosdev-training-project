...
function createNewUser(id: number): UserData {
  ...
  return {
    // id: id.toString(),
    name: name,
    leader: Math.round(Math.random() * 100).toString(),
    count: emails[Math.round(Math.random() * (emails.length - 1))],
    description: emails[Math.round(Math.random() * (emails.length - 1))]
  };
}
